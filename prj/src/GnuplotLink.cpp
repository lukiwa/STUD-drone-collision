...
void GnuplotLink::ShowErrorMessages(bool mode) { display_error_messages_ = mode; }
...
//
bool GnuplotLink::AddFilename(const char *filename, LineStyle style, int width) {
    bool result = true;
...
bool GnuplotLink::CreateChildProcess() {
    ...
    if (pipe(input_descriptor) == -1 || pipe(output_descriptor) == -1) {
        ErrorMessage("### Niemozliwe utworzenie kanalow komunikacyjnych do podprocesow.");
        exit(1);
    }

    switch (fork()) {
        case -1:
            ErrorMessage("### Niemozliwe rozwidlenie procesu.");
            exit(1);
        case 0:
            /* Start potomka */
            ...
            if (close(STDOUT) == -1) {
                ErrorMessage("### Blad zamkniecia kanalu standardowego kanalu wyjsciowego.");
                exit(1);
            }
 ...
 */
bool GnuplotLink::AddDrawingFromFiles(std::string &command, char const **separator) {
    ...
    for (; names != files_information_.end(); ++names) {
        ...
        if (names->GetStyle() == LS_CONTINUOUS)
            OSStrm << " lw " << names->GetWidth();
        else
            OSStrm << " ps " << names->GetWidth();
        command += OSStrm.str();
        *separator = ", ";
    }
    return true;
...
void GnuplotLink::CreateCommandPreamble(std::string &Preamble) const {
    switch (drawing_mode_) {
        case DM_2D:
            Create2DPreamble(Preamble);
            break;
        case DM_3D:
            Create3DPreamble(Preamble);
            break;
    }
}
...
std::string GnuplotLink::SaveRangeSettings(char axis) const {
    ...
    switch (axis) {
        ...
        case 'z':
            ...
            break;
        default:
            cerr << "!!! Blad w 'SaveRangeSettings' niedozwolony"
                    " znak: "
                 << axis << endl;
            return "";
    }
...
std::string GnuplotLink::SaveScaleRotationSettings() const {
    ostringstream strm;
    strm << "set view " << RotationX() << "," << RotationZ() << "," << ScaleX() << "," << ScaleZ()
         << endl;
    return strm.str();
}
...
//
void GnuplotLink::DeleteAllNames() { files_information_.clear(); }
